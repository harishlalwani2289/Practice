package practice.harish.ticketgen.v4;
...
public class TicketGenerator {
    ...
    public static int[][] ticket = new int[6][9];
    ...
    private static void sortAllColumnsInTicket(int[][] ticket) {
        for(int columnNumber=0; columnNumber < ticket[0].length ; columnNumber++) {
            ...
            // Get all the numbers in the positions filled in this column
            List<Integer> numbersInColumn  = new ArrayList<Integer>();
            for(int rowNumber = 0; rowNumber < ticket.length ; rowNumber++ ) {
                if(ticket[rowNumber][columnNumber] != 0)
                    numbersInColumn.add(ticket[rowNumber][columnNumber]);
            }

            // We sort the list of numbers we have obtained
            Collections.sort(numbersInColumn);

            for(int i=0; i<positionsFilledInColumn.size() ; i++) {
                ticket[positionsFilledInColumn.get(i)][columnNumber] = numbersInColumn.get(i);
            }
