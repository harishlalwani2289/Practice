package practice.harish.ticketgen.v1;
...
public class TicketGenerator {
    ...
    private static boolean columnsAreInSortedOrder(int[][] ticket) {
        for(int i =0 ; i < ticket[0].length ; i++) {
            if(!(ticket[0][i] > ticket[1][i] &&  ticket[0][i] > ticket[2][i] && ticket[1][i] > ticket[2][i]))
                return false;
        }
        return true;
    }
    ...
    private static Integer findNextNumberBetween(int min, int max,int[][] fullTicket) {
        Random random = new Random();
        int nextNumber  = random.nextInt(max - min) + min + 1;
        while (ticketContains(nextNumber, fullTicket)) {
            nextNumber  = random.nextInt(max - min) + min + 1;
        }
        return nextNumber;
    }
