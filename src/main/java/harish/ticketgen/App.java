...
import harish.ticketgen.model.Prize;
...
class App extends JFrame implements ActionListener{
 ...
 List<harish.ticketgen.Ticket> ticketsWon;
 ...
 public void addElements(){
  ...
  harish.ticketgen.ExcelNumberHighlight.highlightThePickedNumber(random_number[pos]+1);
 ...
 private void printtheTheTicketsWhichHasWon(List<Integer> calledNumbers, List<harish.ticketgen.Prize> prizeList) {
  ticketsWon = harish.ticketgen.GameWinner.getWinningTickets(calledNumbers, prizeList);
  if(ticketsWon.size() > 0) {

   for(harish.ticketgen.Ticket ticket : ticketsWon) {
